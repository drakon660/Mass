        ...
        public AllocationStateMachine()
        {
            ...
            Initially(
                When(AllocationCreated)
                    ...
                    .TransitionTo(Allocated),When(ReleaseRequested).TransitionTo(Released));
            ...
            During(Released,When(AllocationCreated).ThenAsync(context =>
            ...
            During(Allocated,
                When(HoldExpiration.Received)
                    .ThenAsync(context =>
                        Console.Out.WriteLineAsync($"Allocation expired {context.Instance.CorrelationId}")).Finalize(),
                When(ReleaseRequested).Unschedule(HoldExpiration).ThenAsync(context =>
                    Console.Out.WriteLineAsync($"Allocation release requested {context.Instance.CorrelationId}")));
